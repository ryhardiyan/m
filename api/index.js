...
const _LUAPIConfig = require('../config');
...
const PTERO_NEST_ID = parseInt(process.env.PTERO_NEST_ID || '');
const PTERO_EGG_ID = parseInt(process.env.PTERO_EGG_ID || '');
const PTERO_LOC_ID = parseInt(process.env.PTERO_LOC_ID || '');
...
function makeSpinner(label) {
  process.stdout.write(`\n${c.cyan('[LUAPI]')} ${c.magenta('~')} ${label}\n`);
  return {
    stop(message, color = 'green') {
      const colorFn = c[color] || ((s) => s);
      process.stdout.write(`${c.cyan('[LUAPI]')} ${colorFn(message)}\n`);
    }
  };
}

function logInfo(message) {
  console.log(`${c.cyan('[LUAPI]')} ${message}`);
}

function logWarn(message) {
  console.warn(`${c.yellow('[LUAPI]')} ${message}`);
}

function logError(message) {
  console.error(`${c.red('[LUAPI]')} ${message}`);
}
...
async function validateApiKey(req) {
  ...
  const isFreeKey = providedApiKey === 'LUAPI';
...
const LUAPI_ALLOWED_IMAGE_EXTENSIONS = new Set(['.jpg', '.jpeg', '.png', '.webp', '.gif', '.bmp', '.avif']);

const LUAPI_ALL_MEDIA_EXTENSIONS = new Set([
...
function isMediaUrl(urlStr) {
  ...
  return ext ? LUAPI_ALL_MEDIA_EXTENSIONS.has(ext) : false;
}

function isImageUrl(urlStr) {
  const ext = getExtFromUrl(urlStr);
  return ext ? LUAPI_ALLOWED_IMAGE_EXTENSIONS.has(ext) : false;
}

function hasMediaLikeKey(keyStr) {
  const leaf = String(keyStr).split('.').pop().toLowerCase();
  return LUAPI_MEDIA_KEYWORDS.some(k => leaf.includes(k));
}

const _SUPA_CFG = _LUAPIConfig.supabase || {};
const _SUPA_URL = (_SUPA_CFG.url || '').replace(/\/$/, '');
const _SUPA_KEY = _SUPA_CFG.serviceKey || '';
const _SUPA_BUCKET = _SUPA_CFG.bucket || '';

function _generateLUAPIFilename(ext) {
  const rand = Math.floor(Math.random() * 100000000).toString().padStart(8, '0');
  return `LUAPI${rand}${ext}`;
}
...
async function uploadToSupabase(urlStr, req) {
  ...
  try {
    ...
    const filename = _generateLUAPIFilename(ext);
...
function hasImageLikeKey(pathString) {
  ...
  return LUAPI_IMAGE_KEYWORDS.includes(leaf) || /(?:image|img|thumb|thumbnail|avatar|photo|picture|cover|poster|banner|wallpaper|preview|screenshot|icon)/i.test(leaf);
}
...
const LUAPI_PAGE_KEYWORDS = ['page', 'page_url', 'pageurl', 'result_page', 'view_page'];

function isPageLikeKey(keyStr) {
  const leaf = String(keyStr).split('.').pop().toLowerCase();
  return LUAPI_PAGE_KEYWORDS.includes(leaf);
}
  ...
  for (let i = 0; i < coverIds.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      const resp = await fetch(`https://api.mangadex.org/cover?${qs}&limit=${BATCH_SIZE}`, {
        headers: { 'User-Agent': 'api-LUAPI/2.0 (+https://api.LUAPI.my.id)' },
...
async function sendJson(res, status, payload, context = '', req) {
  ...
  if (finalPayload && typeof finalPayload === 'object' && !Array.isArray(finalPayload)) {
    ...
    responsePayload = { ok: okField !== undefined ? okField : true, creator: 'LUAPI', ...rest };
  } else {
    responsePayload = { ok: true, creator: 'LUAPI', result: finalPayload };
  }
...
async function handler(req, res) {
  try {
    ...
    if (parts[0] !== 'api') {
      ...
      if (hasExt && req.method === 'GET' && /^LUAPI[a-z0-9]+\.[a-z0-9]{1,6}$/i.test(filenameCandidate)) {
    ...
    if (parts[1] === 'create-panel' && req.method === 'POST') {
        ...
        const email = `${username}@LUAPI.dev`;
        ...
        try {
            ...
            const serverRes = await axiosModuleInstance.post(
                ...
                {
                    name,
                    description: `Created via LUAPI API [${new Date().toISOString()}]`,
                    user: user.id,
...
if (parts[1] === 'anime') {
  ...
  try {
    ...
    if (parts[1] === 'manhwa-chapters') {
      ...
      try {
        ...
        const firstR = await fetch(
          ...
          { headers: { 'User-Agent': 'api-LUAPI/2.0 (+https://api.LUAPI.my.id)' }, signal: AbortSignal.timeout(10000) }
        );
    ...
    if (parts[1] === 'docs') {
      ...
      _cachedDocsPayload = { ok: true, creator: 'LUAPI', data: { categories } };
      res.setHeader('Content-Type', 'application/json; charset=utf-8');
      res.setHeader('Cache-Control', 'public, max-age=30');
      return res.status(200).json(_cachedDocsPayload);
    }
    ...
    if (parts[1] === 'ai-agent') {
      const apikey = req.query?.apikey || 'LUAPI';
      const runPath = req.query?.run || null;
      const baseUrl = `https://${req.headers.host || 'api.LUAPI.my.id'}`;
      ...
      if (runPath) {
        try {
          ...
          return res.status(200).send([
            `=== LUAPI API AGENT: EXECUTION RESULT ===`,
            `Endpoint : ${runPath}`,
            `Method   : ${method}`,
      ...
      const lines = [
        `=== LUAPI API - AI AGENT DOCUMENTATION ===`,
        `Base URL : ${baseUrl}`,
        `API Key  : ${apikey}`,
        ``,
...
if (parts[1] === 'tourl' && parts[2] === 'upload' && req.method === 'POST') {
  ...
  async function _tourlImgbb(buffer, filename, mime) {
    ...
    const r = await axiosModuleInstance.post(TOURL_IMGBB_ENDPOINT, form, {
      headers: { ...form.getHeaders(), 'User-Agent': 'LUAPIAPI/2.0' },
  ...
  async function _tourlCatbox(buffer, filename, mime) {
    ...
    const headers = { ...form.getHeaders(), 'User-Agent': 'LUAPIAPI/2.0' };
  ...
  return new Promise((resolve) => {
    uploadMiddleware.single('file')(req, res, async (err) => {
      ...
      const generatedFilename = `LUAPI${Math.floor(Math.random() * 100000000).toString().padStart(8, '0')}${fileExtString}`;
...
'<html lang="id"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Kode OTP LUAPI API</title></head>' +
...
'<div style="font-family:Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:4px;color:#52525b;margin-bottom:10px;">LUAPI</div>' +
'<div style="font-size:22px;font-weight:900;letter-spacing:2px;color:#f4f4f5;">VERIFIKASI LOGIN</div>' +
'</td></tr>' +

'' +
'<tr><td style="padding:28px 32px;">' +
'<p style="color:#71717a;font-size:13px;margin:0 0 24px;line-height:1.8;">Ada permintaan login ke akun <span style="color:#f4f4f5;font-weight:700;">LUAPI</span> kamu. Gunakan kode di bawah untuk melanjutkan. Berlaku <span style="color:#f4f4f5;font-weight:700;">5 menit</span>.</p>' +
...
'<p style="color:#3f3f46;font-size:11px;margin:0;">&#169; 2025 LUAPI &middot; RyharDev</p>' +
'</td></tr>' +
